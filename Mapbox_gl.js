...
mapboxgl.accessToken = 'pk.eyJ1IjoicGVkcm9sc21hcmlhbm8iLCJhIjoiY2t5Mzh2ZDdmMG9lZzJ1bGpkOXFlcHU0ZSJ9.mLnzoqbQMrdZlUuOeB6j0g';
...
function addMapLayers() {
    ...
    if (!map.getLayer("polution_heat_fill")) {
        map.addLayer({
           ...
           //  minzoom: 12, // PEDRO EXPORT HEATMAP IMAGE
            layout: {},
            paint: {
                'fill-color': ['get', 'color'],
    ...
    if (!map.getLayer("polution_heat")) {
        map.addLayer({
            ...
            source: 'polution',
            // minzoom: 12, // PEDRO EXPORT HEATMAP IMAGE
            // maxzoom: 19, // PEDRO EXPORT HEATMAP IMAGE
            paint: {
...
map.on('click', function (e) {
    ...
    var tmp = map.queryRenderedFeatures(e.point);
...
function startAll() {
    ...
    canvas_container.addEventListener('mousedown', function () {
        mouseDown(event, map, canvas);
    }, true);
    ...
    const template = [
        {
            ...
            submenu: [
                ...
                {
                    label: 'Create Pollution Heat Map',
                    click: createHeatMapCSVFile
                },
                {
                    role: "close"
                }
...
const csvWriter = createCsvWriter({
    ...
    header: ['longitude', 'latitude', 'polution', 'tag', 'profiles']
});
...
function generateRandomPoints_helper(iters) {
    ...
    for (var i = 0; i < iters; i++) {
        ...
        //procurar cores extremas mais pr�ximas
        ...
        //interpola��o de valores pollution com weight t;
...
// properties of the region to be exported
const MIN_LONGITUDE = -9.300506;
const MAX_LONGITUDE = -9.078063;
const MIN_LATITUDE = 38.681756;
const MAX_LATITUDE = 38.813035;
const CELL_LONGITUDE_SIZE = 0.000630859374999;
const CELL_LATITUDE_SIZE = 0.000557031249997;

function createHeatMapCSVFile__ ()
{
    const gl = canvas.getContext('webgl') || canvas.getContext('webgl2');
    if (!gl)
        return;
    // save current camera map position
    const camera_center = map.getCenter();
    const camera_zoom = map.getZoom();
    const camera_bearing = map.getBearing();
    // place the camera so that we can see the region to be exported
    const BOUNDS = [[MIN_LONGITUDE, MIN_LATITUDE], [MAX_LONGITUDE, MAX_LATITUDE]];
    map.fitBounds (BOUNDS);
    // show the heatmap
    var heatmap_check = document.getElementById("heatmap_check");
    var heatmap_slider = document.getElementById("heatmap_slider");
    
    const HEATMAP_CHECKBOX_IS_CHECKED = heatmap_check.checked;
    const HEATMAP_SLIDER_VALUE = heatmap_slider.value;
    ...
    map.setPaintProperty("polution_heat_fill", "fill-opacity", 1);
    // create heatmap file
    const filename = path.join ('.', path.join ('Data', 'HeatMap.dat'));
    console.log (filename);
    var file = fs.openSync (filename, 'w');
    for (var longitude = MIN_LONGITUDE; longitude < MAX_LONGITUDE; longitude += CELL_LONGITUDE_SIZE) {
        var first = true;
        for (var latitude = MIN_LATITUDE; latitude < MAX_LATITUDE; latitude += CELL_LATITUDE_SIZE) {
            if (first)
                first = false;
            else
                fs.writeSync (file, ' ');
            const p = computePollutionForHeatMap_ (longitude, latitude, gl);
            fs.writeSync (file, p);
        }
        fs.writeSync (file, '\n');
    }
    fs.closeSync (file);
    // restore camera map position
    map.setCenter (camera_center);
    map.setZoom (camera_zoom);
    map.setBearing (camera_bearing);
    // restore heatmap properties
    heatmap_check.checked = HEATMAP_CHECKBOX_IS_CHECKED;
    heatmap_slider.value = HEATMAP_SLIDER_VALUE;
    map.setPaintProperty("csv_tmp_layer", "background-opacity", 0);
}

function createHeatMapCSVFile_ ()
{
    const gl = canvas.getContext('webgl') || canvas.getContext('webgl2');
    if (!gl)
        return;
    if (!is_heatmap_displayed)
...
function computePollutionForHeatMap_ (longitude, latitude, gl)
{
    const point = map.project ([longitude, latitude]);
    const rgb = [];
    const data = new Uint8Array(4);
    const canvasX = point.x - canvas.offsetLeft;
    const canvasY = canvas.height - point.y - canvas.offsetTop;
    gl.readPixels(canvasX, canvasY, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, data);
    rgb.push(data[0]);
    rgb.push(data[1]);
    rgb.push(data[2]);
    var p = {
        ...
        g: rgb[1],
        b: rgb[2] };
    // search nearest colours
    var index, min, first = true;
    for (var j in heatmap_color_vectors) {
        var p1 = {
            r: heatmap_color_vectors[j].vec[0][0],
            g: heatmap_color_vectors[j].vec[0][1],
            b: heatmap_color_vectors[j].vec[0][2] };
        var p2 = {
            r: heatmap_color_vectors[j].vec[1][0],
            g: heatmap_color_vectors[j].vec[1][1],
            b: heatmap_color_vectors[j].vec[1][2] };
        var tmp = pointToLineDist(p, p1, p2);
        if (tmp.dist < min || first) {
            min = tmp.dist;
            obj = tmp;
            index = j;
            first = false;
    ...
    //interpolate pollution values;
    const top = heatmap_color_vectors[index].pol1;
    const bottom = heatmap_color_vectors[index].pol2;
    const distance = bottom - top;
    result = top + (obj.weight * distance);
    return result;
}

function createHeatMapCSVFile ()
{
    // create heatmap file
    const filename = path.join ('.', path.join ('Data', 'HeatMap.dat'));
    console.log (filename);
    ...
    for (var longitude = MIN_LONGITUDE; longitude < MAX_LONGITUDE; longitude += CELL_LONGITUDE_SIZE) {
        ...
        for (var latitude = MIN_LATITUDE; latitude < MAX_LATITUDE; latitude += CELL_LATITUDE_SIZE) {
            if (first)
                ...
                fs.writeSync (file, ' ');
            const p = computePollutionForHeatMap (longitude, latitude);
            fs.writeSync (file, p);
        }
        fs.writeSync (file, '\n');
    }
    fs.closeSync (file);
    log.info ("Created heat map");
}

/**
 * Computes the pollution level at the given location.
 * Loops through all polution sources stored at variable heatmap_features and adds all contributions that are under the pollution source maximum range.
 * Pollution is modelled by a gaussian distribution.
 * @param {Number} longitude
 * @param {Number} latitude
 * @returns {Number}
 */
function computePollutionForHeatMap (longitude, latitude)
{
    var result = 0;
    for (var _if in heatmap_features.features) {
        const feature = heatmap_features.features [_if];
        const max_distance = feature.properties.range_base;
        var found = false;
        var feature_distance;
        if (feature.geometry.type === "Point") {
            feature_distance = distanceSurfaceEarth (longitude, latitude, feature.geometry.coordinates);
            found = feature_distance < max_distance;
        }
        else if (feature.geometry.type === "MultiPoint") {
            var first = true;
            for (var _ic in feature.geometry.coordinates) {
                const coordinate = feature.geometry.coordinates [_ic];
                const this_distance = distanceSurfaceEarth (longitude, latitude, coordinate);
                if (first) {
                    feature_distance = this_distance;
                    first = false;
                } else
                    feature_distance = Math.min (feature_distance, this_distance);
            }
            found = feature_distance < max_distance;
        }
        else {
            log.info ("Do not know how to process:");
            log.info (feature.geometry);
        }
        if (found) {
            const pollution_contribution = feature.properties.level * Math.exp (
                    - 4 * feature_distance * feature_distance / max_distance / max_distance);
            result += pollution_contribution;
        }
    }
    return result;
}
...
//    for (var _if in heatmap_features.features) {
//        const feature = heatmap_features.features [_if];
//        const max_squared_distance = feature.properties.range_base * feature.properties.range_base;
//        var found = false;
//        var feature_squared_distance;
//        if (feature.geometry.type === "Point") {
//            feature_squared_distance = squaredDistanceSurfaceEarth (longitude, latitude, feature.geometry.coordinates);
//            found = feature_squared_distance < max_squared_distance;
//        }
//        else if (feature.geometry.type === "MultiPoint") {
//            var first = true;
//            for (var _ic in feature.geometry.coordinates) {
//                const coordinate = feature.geometry.coordinates [_ic];
//                const this_squared_distance = squaredDistanceSurfaceEarth (longitude, latitude, coordinate);
//                if (first) {
//                    feature_squared_distance = this_squared_distance;
//                    first = false;
//                } else
//                    feature_squared_distance = Math.min (feature_squared_distance, this_squared_distance);
//            }
//            found = feature_squared_distance < max_squared_distance;
//        }
//        else {
...
//        if (found) {
//            console.log (feature);
//            const pollution_contribution = feature.properties.level * Math.exp (- feature_squared_distance / max_squared_distance / 2);
//            console.log ("Pollution " + pollution_contribution + " of " + feature.properties.level);
//            result += pollution_contribution;
//        }
//    }
//    return result;
...
function distanceSurfaceEarth (longitude, latitude, coordinate)
{
    const R = 6371e3; // metres
    const φ1 = latitude * Math.PI/180; // φ, λ in radians
    const φ2 = coordinate [1] * Math.PI/180;
    const Δφ = (coordinate [1] - latitude) * Math.PI/180;
    const Δλ = (coordinate [0] - longitude) * Math.PI/180;
...
function squaredDistance (longitude, latitude, coordinate)
{
    const x = longitude - coordinate [0];
    const y = latitude - coordinate [1];
    return x * x + y * y;
}

// variables used by the function toggleHeatMapDisplay 
var is_heatmap_displayed = false;
var display = {};
...
function toggleHeatMapDisplay ()
{
    ...
    if (is_heatmap_displayed) {
        // restore camera map position
        map.setCenter(display.camera_center);
        map.setZoom(display.camera_zoom);
        map.setBearing(display.camera_bearing);
        map.setPitch(display.camera_pitch);
        // restore heatmap properties
        heatmap_check.checked = display.heatmap_checkbox_is_checked;
        heatmap_slider.value = display.heatmap_slider_value;
        map.setPaintProperty("csv_tmp_layer", "background-opacity", display.map_csv_tmp_layer_background_opacity);
        map.setPaintProperty("polution_heat", "heatmap-opacity", display.map_polution_heat_heatmap_opacity);
        map.setPaintProperty("polution_heat_fill", "fill-opacity", display.map_polution_heat_fill_fill_opacity);
        is_heatmap_displayed = false;
    } else {
        display = {
            camera_center: map.getCenter(),
            camera_zoom: map.getZoom(),
            camera_bearing: map.getBearing(),
            camera_pitch: map.getPitch(),
            heatmap_checkbox_is_checked: heatmap_check.checked,
            heatmap_slider_value: heatmap_slider.value,
            map_csv_tmp_layer_background_opacity: map.getPaintProperty ("csv_tmp_layer", "background-opacity"),
            map_polution_heat_heatmap_opacity: map.getPaintProperty("polution_heat", "heatmap-opacity"),
            map_polution_heat_fill_fill_opacity: map.getPaintProperty("polution_heat_fill", "fill-opacity")
        };
        // place the camera so that we can see the region to be exported
        const BOUNDS = [[MIN_LONGITUDE, MIN_LATITUDE], [MAX_LONGITUDE, MAX_LATITUDE]];
        map.setPitch (0);
        ...
        map.setPaintProperty("polution_heat", "heatmap-opacity", 1);
        map.setPaintProperty("polution_heat_fill", "fill-opacity", 1);
        is_heatmap_displayed = true;
    }
    
    console.log (heatmap_features);
    console.log (heatmap_circles);
    console.log (heatmap_fill_features);
}